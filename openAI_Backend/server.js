...
// // Keep track of first-time emoji usage per emotion
// const shownEmoji = new Set();

// // Store chat history in memory (for simplicity; consider DB for production)
// let conversationHistory = [];
...
//           {
//             role: "system",
//             content: "Classify the user's message into exactly one emotion from this list: happy, sad, angry, scared, surprised, confused, lonely, excited, frustrated or neutral. Respond with only the emotion word."
//           },
//           { role: "user", content: userMessage }
//         ],
//         max_tokens: 5,
...
//     // ---------------- Step 4: Apply Emoji Rules ----------------
//     const firstEmojiEmotions = ["happy", "excited", "surprised", "sad", "scared", "lonely", "frustrated"];
//     let replies = [];

//     if (firstEmojiEmotions.includes(emotion) && !shownEmoji.has(emotion)) {
//       // First-time message of this emotion → separate emoji + reply
//       replies = [emoji, reply];
//       shownEmoji.add(emotion);
//     } else {
//       // Subsequent messages → normal reply with 80% chance of inline emoji
//       if (Math.random() < 0.8) {
//         reply = `${reply} ${emoji}`;
//       }
//       replies = [reply];
